...
//
// 1) Configure the render server URL and thumbnail size through the profile
//    patch config (cordis.patch.yml), resolved HERE on the Host and shipped to
//    the browser as boot-time globals:
//
...
/** Default render server when no override is configured (unused by the client; kept for reference). */
 ...
 */
function injectBootGlobals(html, enableLocalRender, fallbackRenderUrl, enableFallback, thumbMaxHeight, version) {
	const script = `<script>window.__MERMAID_ENABLE_LOCAL_RENDER__ = ${enableLocalRender};window.__MERMAID_FALLBACK_RENDER_URL__ = ${JSON.stringify(fallbackRenderUrl)};window.__MERMAID_ENABLE_FALLBACK__ = ${enableFallback};window.__MERMAID_THUMB_MAX_HEIGHT__ = ${thumbMaxHeight};window.__MERMAID_IMAGE_PREVIEW_VERSION__ = ${JSON.stringify(version)}<\/script>`;
 ...
 * @param config - row config from cordis.patch.yml (may carry enableLocalRender / fallbackRenderUrl / enableFallback / thumbMaxHeight).
 */
export function apply(ctx, config) {
	const enableLocalRender = resolveEnableLocalRender(config);
	ctx.inject(["webServer"], (httpCtx) => {
		httpCtx.effect(() => httpCtx.webServer.tapIndex((html) => injectBootGlobals(html, enableLocalRender, resolveFallbackRenderUrl(config), resolveEnableFallback(config), resolveThumbMaxHeight(config), PLUGIN_VERSION)), "@baconbao/dsh-mermaid-image-preview: boot globals bootstrap");
