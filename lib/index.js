// dsh-mermaid-preview-plugin - node half.
...
export function apply(ctx, config) {
	ctx.inject(["webServer"], (httpCtx) => {
		httpCtx.effect(() => httpCtx.webServer.tapIndex((html) => injectBootGlobals(html, resolveMermaidInkUrl(config), resolveThumbMaxHeight(config))), "dsh-mermaid-preview-plugin: boot globals bootstrap");
	});
}
