...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = ".dsh-mermaid-tail{flex-direction:column;gap:4px;margin:0 0 2px;display:flex}.dsh-mermaid-toggle{width:max-content;color:var(--dsw-alias-label-secondary);cursor:pointer;background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l1);border-radius:6px;padding:2px 10px;font-size:12px;line-height:20px}.dsh-mermaid-toggle:hover{color:var(--dsw-alias-label-primary)}.dsh-mermaid-box{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);border-radius:8px;padding:6px 8px;justify-content:center;align-items:flex-start;display:flex}.dsh-mermaid-thumb{max-width:100%;max-height:204px;width:auto;height:auto;display:block;cursor:zoom-in}.dsh-mermaid-full{max-width:100%;width:auto;height:auto;display:block;cursor:zoom-out}.dsh-mermaid-loading{color:var(--dsw-alias-label-secondary);font-size:13px;line-height:20px}.dsh-mermaid-error{color:var(--dsw-alias-state-error-primary);font-size:12px;line-height:18px;margin-bottom:4px}.dsh-mermaid-fallback{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);border-radius:10px;padding:10px 12px;overflow-x:auto}.dsh-mermaid-fallback pre{margin:0;white-space:pre-wrap;word-break:break-word;font:13px/20px var(--ds-font-family-code);color:var(--dsw-alias-label-primary)}";
		const tagId = "dsh-mermaid-image-preview/tail.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "dsh-mermaid-image-preview";
		...
		// and dsh restarts, and shared through a module-level store so every
		// rendered turn tail stays in sync when any toggle is clicked.

		/** localStorage key for the on/off preference. */
		const VIEW_ENABLED_KEY = "dsh-mermaid-image-preview.enabled";

		/** Current on/off state (module-level store). */
		let viewEnabled = readPersistedViewEnabled();

		/** Listeners notified on every toggle (module-level). */
		const viewEnabledListeners = new Set();

		/** Read the persisted preference, defaulting to enabled when unset. */
		function readPersistedViewEnabled() {
			try {
				const stored = typeof localStorage !== "undefined" ? localStorage.getItem(VIEW_ENABLED_KEY) : null;
				if (stored !== null) return stored !== "false";
			} catch (e) {
				// storage unavailable (e.g. private mode) - default to enabled
			}
			return true;
		}

		/** @returns the current on/off state. */
		function getViewEnabled() {
			return viewEnabled;
		}

		/** Subscribe to toggle changes; returns a disposer. */
		function subscribeViewEnabled(listener) {
			viewEnabledListeners.add(listener);
			return () => viewEnabledListeners.delete(listener);
		}

		/** Flip the on/off state, persist it, and notify all subscribers. */
		function setViewEnabled(enabled) {
			viewEnabled = enabled;
			try {
				if (typeof localStorage !== "undefined") localStorage.setItem(VIEW_ENABLED_KEY, String(enabled));
			} catch (e) {
				// storage unavailable - preference is session-only
			}
			for (const listener of viewEnabledListeners) listener();
		}

		/** React hook: current on/off state, re-rendered on any toggle. */
		function useViewEnabled() {
			const [enabled, setEnabled] = react.useState(getViewEnabled);
			react.useEffect(() => subscribeViewEnabled(() => setEnabled(getViewEnabled())), []);
			return enabled;
		}
		...
		function MermaidTurnTail(props) {
			...
			const enabled = useViewEnabled();
			if (!diagrams.length) return null;
			// Toggle the global switch: persists the preference and notifies
			// every rendered turn tail so they all follow instantly.
			const toggle = () => setViewEnabled(!enabled);
			const toggleButton = react.createElement("button", {
				className: "dsh-mermaid-toggle",
				onClick: toggle,
			}, enabled ? "Disable dsh-mermaid-image-preview" : "Enable dsh-mermaid-image-preview");
			if (!enabled) {
				return react.createElement("div", { className: "dsh-mermaid-tail" }, toggleButton);
			}
