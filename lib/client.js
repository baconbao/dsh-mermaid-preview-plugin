...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// Read the thumbnail max height: persisted settings first (editable in
		// the settings page), else the Host-injected boot global (config), else 204.
		function readThumbMaxHeight() {
			if (typeof settings !== "undefined" && typeof settings.thumbMaxHeight === "number") {
				return settings.thumbMaxHeight;
			}
		...
		function MermaidDiagram({ code, scheme, localRenderUrl, enableLocalRender, fallbackRenderUrl, enableFallback, thumbMaxHeight }) {
			...
			react.useEffect(() => {
				...
				probe.src = localUrl;
				return () => { alive = false; };
			}, [code, scheme, enableLocalRender, fallbackRenderUrl, enableFallback]);
		...
		function apply(ctx) {
			...
			// Settings-page card (additive; a fresh id joins the shipped cards).
